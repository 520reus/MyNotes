...
PathTimeGraph::PathTimeGraph(
  ...
  SetupObstacles(obstacles, discretized_ref_points); //在构造函数中，给S-T graph添加所有的动静态障碍物
}
...
std::vector<std::pair<double, double>> PathTimeGraph::GetLateralBounds(
  ...
  for (size_t i = 0; i < bounds.size(); ++i) {
    //take the ego width into consideration, the vehiche shouldn't collide with
    //lane edge
...
/**
 * @brief adjust lateral bounds based on obstacles' relative position to
 *        lane center line
...
void PathTimeGraph::UpdateLateralBoundsByObstacle(
  ...
  if (sl_boundary.end_l() < FLAGS_numerical_epsilon) {
    for (size_t i = start_index; i < std::min(end_index + 1, bounds->size());
         ++i) {
      //adjust right bound to be more left, left nudge, but in the lane,
      //don't change lane
  ...
  if (sl_boundary.start_l() > -FLAGS_numerical_epsilon) {
    for (size_t i = start_index; i < std::min(end_index + 1, bounds->size());
         ++i) {
      //adjust left bound to be more right, right nudge, but in the lane,
      //don't change lane
