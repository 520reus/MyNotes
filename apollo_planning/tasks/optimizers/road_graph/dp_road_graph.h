...
/**
 * DP-Path的主要思路是以自车当前位置为起点，沿着车道横纵向（横向为L或d，纵向为s，
 * 这里为了字母更清晰，横向统一以d表示）采样一些点，横向采样的一组点叫做level，
 * 点封装成node后，分别计算不同level间的node的cost，就构成了graph。利用DP更新node的最小cost，
 * 便找到了代价最小的一条路径。听起来很像传统的图搜索方法找最短路径，区别在于cost包含了平滑、无碰的指标。 
 */
...
class DpRoadGraph {
 public:
  ...
  //所有构造graph、计算cost、search的过程都在FindPathTunnel()中。
 ...
 private:
  ...
  struct DpRoadGraphNode {
   public:
    ...
    //该函数用于在寻找最小cost过程中更新每个当前节点与之连接的前层节点的指针、curve以及更新起点到该点的cost
