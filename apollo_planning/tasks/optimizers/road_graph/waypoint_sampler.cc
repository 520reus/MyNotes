...
/**
 * @file 对道路前方40米进行分层采样
 * @param init_point,车辆当前位置
 * @param points,处理后的所有采样点都保存在points中，points为二维数组，每一个元素为一层level中的采样点集
 * @return bool,采样是否成功
 */
bool WaypointSampler::SamplePathWaypoints(
    const common::TrajectoryPoint &init_point,
    std::vector<std::vector<common::SLPoint>> *const points) {//指针常量，points指向传入的变量地址且地址不可变
  CHECK_NOTNULL(points);
  points->clear();
  points->insert(points->begin(), std::vector<common::SLPoint>{init_sl_point_});

  const double kMinSampleDistance = 40.0;  //纵向采样距离
  const double total_length = std::fmin( //返回两个参数中较小的那一个
      init_sl_point_.s() + std::fmax(init_point.v() * 8.0, kMinSampleDistance),
      reference_line_info_->reference_line().Length());
  const auto &vehicle_config =
      common::VehicleConfigHelper::Instance()->GetConfig();
  const double half_adc_width = vehicle_config.vehicle_param().width() / 2.0; //车宽的一半
  const double num_sample_per_level =  //每层的采样点个数
      FLAGS_use_navigation_mode ? config_.navigator_sample_num_each_level() //使用导航模式的采样点个数
                                : config_.sample_points_num_each_level();  //不使用导航模式的采样点个数

  static constexpr double kSamplePointLookForwardTime = 4.0;
  const double level_distance = //层与层采样点间的距离
      //math::Clamp()为区间限定函数，在区间内返回原值，比限定的最值小(大)则返回最小(大)值
  ...
  static constexpr size_t kNumLevel = 3; //层数
