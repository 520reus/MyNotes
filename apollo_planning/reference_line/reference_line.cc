...
bool ReferenceLine::Stitch(const ReferenceLine& other) {
  ...
  bool first_join = first_sl.s() > 0 && first_sl.s() < other.Length();//当前参考线在待缝合参考线“前”面
  ...
  bool last_join = last_sl.s() > 0 && last_sl.s() < other.Length();//当前参考线在待缝合参考线“后”面
  ...
  if (first_join) {
    if (first_sl.l() > kStitchingError) { //若开始缝合点的横向误差>0.1,则不再考虑缝合
      AERROR << "lateral stitching error on first join of reference line too "
                "big, stitching fails";
      return false;
    }
    //std::lower_bound二分查找，返回第一个出现的位置
...
/**
 * @brief 以下两个函数用于在pnc_map所提供的全局参考路径上，找到自车位置后方30米，前方180或250米范围的局部路径
 * @param point 给定自车位置点
 * @param look_backward 后视范围，30米
 * @param look_forward 前视范围，180米或250米
 * @return 是否可以找到自车前后对应范围的一段参考线
  */
...
/**
 * @brief 给定自车或障碍物点，从XY系投影到参考线上，返回相应的SL系下的点
 * @param path_point 自车或障碍物点[x,y,theta,kappa,s..]
 * @return SL系下的点[s,l,dl,ddl]
  */
...
//TODO 注意该函数与ReferencePoint ReferenceLine::GetNearestReferencePoint(const double s) const的区别
//TODO 目前尚未搞明白，但是被很多函数使用，所以很重要
/**
 * @brief 
 */
...
/**
 * @brief SL系下的点映射到XY系
 * @param sl_point 输入的sl点[s,l]
 * @param xy_point 映射后的xy点[x,y]
 * @return  是否能够完成映射
 */
...
/**
 * @brief XY系下的点映射到SL系
 * @param xy_point 输入的xy点[x,y]
 * @param sl_point 映射后的sl点[s,l]
 * @return  是否能够完成映射
 */
