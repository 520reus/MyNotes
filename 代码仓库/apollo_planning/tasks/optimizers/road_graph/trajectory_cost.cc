...
TrajectoryCost::TrajectoryCost(const DpPolyPathConfig &config,
  ...
  //时间戳的数量，决定了需要考虑多少个时间戳的动态物体的瞬时状态？还是自车的状态
  ...
  for (const auto *ptr_obstacle : obstacles) {
    ...
    const auto &sl_boundary = ptr_obstacle->PerceptionSLBoundary(); //障碍物在SL系下的多边形大小/范围
    // 当前车辆最左侧的l位置(左正右负)
    ...
    //lateral_ignore_buffer:If an obstacle's lateral distance is further away than this distance, ignore it
    // 1° 障碍物在车辆左侧，并且超出考虑范围
    // 2° 障碍物在车辆右侧，并且超出考虑范围
    ...
    if (ptr_obstacle->IsVirtual()) {//虚拟障碍物
      // Virtual obstacle
      continue;
    } else if (ptr_obstacle->IsStatic() || is_bycycle_or_pedestrian) { //静态障碍物
      ...
      for (uint32_t t = 0; t <= num_of_time_stamps_; ++t) {
        ...
        static constexpr double kBuff = 0.5; //对障碍物进行一个kBuff尺寸的碰撞，更好的避免碰撞
...
//path cost包含了论文中的C_smooth(只使用了一二阶导，没有使用三阶导)，又包含了C_guidance（其实公式中的g(s)
//就是reference line，f(s)-g(s)就是横向坐标l）。除此之外，还考虑了起点与终点的横向偏移。
ComparableCost TrajectoryCost::CalculatePathCost(
  ...
  path_cost *= config_.path_resolution(); //TODO 这里为什么要乘离散分辨率呢？
  //如果是最后一层采样点，path_cost还要加上起点与终点的横向偏移，该值越小越好
...
bool TrajectoryCost::IsOffRoad(const double ref_s, const double l,
  ...
  Vec2d rear_center(0.0, l); //车辆后轴中心SL坐标点初始化
...
//计算当前采样点自车位姿与所有静态障碍物之间的sum cost
...
// 计算当前采样点车辆位姿与所有动态障碍物之间的sum cost
ComparableCost TrajectoryCost::CalculateDynamicObstacleCost(
  ...
  for (size_t index = 0; index < num_of_time_stamps_;
    ...
    common::SpeedPoint speed_point; //纵向速度点信息[s,t,v,a,da]
    heuristic_speed_data_.EvaluateByTime(time_stamp, &speed_point); //根据时间戳获取自车速度点信息
    double ref_s = speed_point.s() + init_sl_point_.s();
    //如果还没到达要考虑的[start_s,end_s]范围，忽略本次循环下面的内容
    if (ref_s < start_s) {
      continue;
    }
    //如果自车在t时刻已经超过[start_s,end_s]这段需要计算cost的范围，则直接跳出循环
    if (ref_s > end_s) {
      break;
    }
...
//计算当前车辆位置与静态障碍物的bounding box boundary (obs_sl_boundary)距离进行计算，
//忽略了中心在参考线附近的障碍物、横向距离自车很远的障碍物、纵向在自车之后的障碍物、横向大于安全距离的障碍物
//这里在计算的时候不考虑车辆位置的heading，自认为是静态障碍物较为安全，但是动态障碍物时会考虑heading
ComparableCost TrajectoryCost::GetCostFromObsSL(
  ...
  //* 在二维SL坐标系下看bounding box有没有相交(碰撞)的简化方法，即分为两个一维问题求解
  ...
  */
  ...
  //不考虑横向大于安全距离的障碍物
  static constexpr double kSafeDistance = 0.6; //安全距离dn，小于安全距离开始计算C_obsNudge
  if (delta_l < kSafeDistance) {
    //合并d>dc与d<dc的情况,config_.obstacle_collision_distance()为dc，
    //config_.obstacle_collision_cost()为C_collision
    //利用Sigmoid函数求解有碰撞可能的两种情况 
    obstacle_cost.safety_cost +=
...
ComparableCost TrajectoryCost::GetCostBetweenObsBoxes(
  ...
  //obstacle_collision_distance是有可能的碰撞距离，设为dc;obstacle_risk_distances是危险距离，设为dr
  //其中dc>dr,当车辆与动态障碍物之间的距离满足dr<d<dc时，有一倍的惩罚系数；当d<dr时，有20倍的惩罚系数
  obstacle_cost.safety_cost +=
      config_.obstacle_collision_cost() *
