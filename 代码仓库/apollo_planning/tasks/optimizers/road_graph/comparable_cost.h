...
class ComparableCost {
 public:
   ...
   */
  // 以下成员变量均有默认初始值，当调用无参构造函数时，成员变量自动初始化为以下值
  ...
  // cost from distance to obstacles or boundaries  ,c_obs
  double safety_cost = 0.0f;   
  // cost from deviation from lane center, path curvature etc   ,c_smooth + c_guidance
