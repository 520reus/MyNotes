...
DT = 0.2  # time tick [s]
MAX_T = 5.0  # max prediction time [m]   预瞄时间，决定每一时刻轨迹的长度，十分重要的参数
...
def calc_global_paths(fplist, csp):
    '''
    将Frenet坐标系转换到笛卡尔全局坐标系
    输出：
    :param fplist: 带有Frenet坐标系参数的轨迹
    :param csp: 参考分段样条曲线
    :return:  完成坐标转换，完善笛卡尔坐标系参数[x,y,yaw,ds,c]
    '''
    for fp in fplist:

        # calc global positions
        for i in range(len(fp.s)):
            ix, iy = csp.calc_position(fp.s[i])  # 中心线参考点的全局位置
            if ix is None:
                break
            i_yaw = csp.calc_yaw(fp.s[i])  # 中心线参考点的航向角
            di = fp.d[i]
            # fx = ix + di * math.cos(i_yaw + math.pi / 2.0)
            # fy = iy + di * math.sin(i_yaw + math.pi / 2.0)
            fx = ix - di*math.sin(i_yaw) # 计算轨迹束上点的全局坐标（以当前中心参考点的位置作为参考来求）
            fy = iy + di*math.cos(i_yaw)
            fp.x.append(fx)
            fp.y.append(fy)
...
def check_paths(fplist, ob):
    '''
    所有轨迹过一遍最大速度、加速度以及碰撞检测，返回通过检测的路径
    :param fplist: 待检测轨迹束
    :param ob: 障碍物集合
    :return: 通过检测的轨迹束
    '''
...
def frenet_optimal_planning(csp, s0, c_speed, c_d, c_d_d, c_d_dd, ob):
    '''
    找到最优的轨迹
    :param csp:     参考分段三次样条曲线
    :param c_speed: current speed [m/s]
    :param c_d:     current lateral position [m]
    :param c_d_d:   current lateral speed [m/s]
    :param c_d_dd:  current lateral acceleration [m/s]
    :param s0:      current course position
    :param ob:      obstacle point list
    :return: 最优轨迹（仅一条）
    '''
...
def main():
    ...
    for i in range(SIM_LOOP):
        ...
        if show_animation:  # pragma: no cover
            ...
            plt.plot(path.x[1:], path.y[1:], "-or")  # 最优轨迹
            plt.plot(path.x[1], path.y[1], "vc")  # 当前位置
