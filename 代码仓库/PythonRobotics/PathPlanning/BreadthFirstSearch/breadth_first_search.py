...
与A*算法不同，BFS从起点开始一圈一圈向外探索，虽然比较傻，但是能保证找到一条最优路径
closed_set中存储的即是遍历过的路径点
open_set中存储没有遍历过的路径点
每次循环从open_set中pop出第一个元素，再根据当前点扩展一些新的路径点加入open_set
直到open_set为空（未找到路径）或找到最终路径为止
...
def main():
    ...
    print("Running time:%0.4s second"%(end-start))
