...
Reeds Shepp曲线用于求解从起点到终点的车辆最短路径，它从有限的状态中选择路径最短的路径拼接序列。
Reeds Shepp采用单车运动学模型，在控制空间采样，但仅有两种控制输入，即最大前轮转角与负最大前轮转角。
与 dubins曲线不同的是，Reeds Shepp允许车辆倒车行驶，即加入一个direction状态，1表示前进，-1表示后退
...
class Path:
    ...
    def __init__(self):
        self.lengths = []  # 比如ctypes=['L','R','L']，表示每段的长度
        self.ctypes = []   # 种类
        self.L = 0.0       # 路径总长度
        # 插值路径点信息
        self.x = []
...
# 先对2pi取余得到范围属于[0,2pi),再转化到(-pi,pi]
def mod2pi(x):
...
def generate_path(q0, q1, max_curvature):
    # 姿态归一化，将起点q0平移到[0,0],对qg采取同样的平移向量[-x0,y0],再旋转-q0.yaw,使得车辆起点朝向转到x轴正向
    # 然后对车辆转向半径放缩到单位1
    dx = q1[0] - q0[0]
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2]
...
def generate_local_course(total_length, lengths, mode, max_curvature, step_size):
    # math.trunc 截断除法，返回整数，而math.floor 地板除法，在python3.5以后返回的也是整数，2.7版本返回为浮点数
...
def calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size):
    ...
    for path in paths:
        ...
        # convert global coordinate (从姿态归一化的坐标系再转化回全局坐标系)
