"""
hybrid A*中用于计算启发式函数h(n)的脚本
hybrid A*在循环搜索前，先计算终点到图中所有节点的h(n)值，（不包括障碍物）
该脚本中计算的h(n)是利用迪杰斯特拉算法，即计算图中每个节点到终点的真实最短距离，而不是估计的2D欧式距离
...
# Node:[在c-space中的位置索引，当前节点到起始节点的总代价，该节点的父节点]
class Node:
...
def calc_distance_heuristic(gx, gy, ox, oy, resolution, rr):
    ...
    # 堆排序的对象必须是列表来表示堆结构，列表中的值可采用元组，实现对带权值的元素进行排序
    # priority_queue储存节点的[cost,index]
...
# 根据输入节点是在全局下的还是栅格图下的，决定障碍物图是全局下的还算栅格图下的
