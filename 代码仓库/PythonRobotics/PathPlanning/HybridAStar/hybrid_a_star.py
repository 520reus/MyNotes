...
class Node:
    ...
    def __init__(self, x_ind, y_ind, yaw_ind, direction,
        ...
        # todo 节点结构中点序列的作用？ 目前推导为其父节点到当前节点的车辆离散状态序列
        self.x_list = x_list
        self.y_list = y_list
...
# 搜索空间离散化
# 构建c-space, Hybird A*算法同时考虑空间连通性和车辆朝向，将二维平面空间和角度同时进行二维离散化。
class Config:
...
# 车辆控制空间离散化
# 车辆的控制输入主要有两个：方向盘转角(Steering Angle)和运动方向(direction)。将方向盘转角从最小转角
# (Min Steering Angle)到最大转角(Max Steering Angle)按照一定间隔进行采样；车辆的运动方向只有两个：向前运动和向后运动。
def calc_motion_inputs():
    # 保证有steer=0的情况，即下一步走直线
...
# 在control space中采样，根据离散化的控制输入积分一段时间推出可行的neighbor node
...
# 计算总代价f(n),f(n)=g(n)+h(n),是priority_queue pop元素的依据
def calc_cost(n, h_dp, c):
    # 需要跟计算h(n)中n的index计算方法一致！否则怎么找到相应的节点呢？
