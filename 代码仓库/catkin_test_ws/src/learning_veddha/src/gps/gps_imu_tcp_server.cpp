...
crss_vdcar::ImuCalib check_data;
using namespace std;
	...
	*/
	...
	if (v.size() >= 24)
	{
		...
		if(v[21]!="")
		{
			...
			if (v[18] != "") VBaseline = std::atof(v[18].c_str());
			//std::cout << "Ve_east:" << Ve  << "\t\tVn_north:" << Vn << "\t\tVu_vertical:" << Vu << "\t\tVu_vertical:" << VBaseline<< std::endl;
			...
			//std::cout << "orien_x:" << Roll << "\torien_y:" << Pitch << "\torien_z:" << Heading << std::endl;						
		}
		else
		{
...
void imu_gps_calibarCallback(const crss_vdcar::ImuCalib msg)
{
	check_data = msg;
}
...
{	
	ros::init(argc, argv, "gps_imu_TCP_server");
	ros::NodeHandle nh;
	...
	ros::Publisher gps_status_raw_pub_  = nh.advertise<crss_vdcar::GpsInfo>("/gps/info_raw", 100);
	ros::Publisher gps_status_pub_      = nh.advertise<crss_vdcar::GpsInfo>("/gps/info", 100);
	ros::Subscriber check_sub_          = nh.subscribe("/imu/calibar", 100, imu_gps_calibarCallback);
	...
	//初始化
	check_data.linearx = 0;
	check_data.lineary = 0;
	check_data.linearz = 0;
	check_data.ThetaX = 0;
	check_data.ThetaY = 0;
	check_data.ThetaZ = 0;    
			...
			{     
				...
				else
				{
					...
					else
					{
						...
						gps_raw_data.header.stamp    = ros::Time::now();
						...
						degRoll    = Roll;             //->X
						degPitch   = Pitch;            //->y
						degHeading = Heading;          //->Z

						crss_vdcar::GpsInfo gps_raw_status;
						gps_raw_status.status  = status;
						gps_raw_status.Vx      = Vn;                   
						gps_raw_status.Vy      = Ve;             
						gps_raw_status.Vz      = Vu;                 
						gps_raw_status.Vv      = VBaseline;
						gps_raw_status.Heading = degHeading;
						gps_raw_status.Pitch   = degPitch;
						gps_raw_status.Roll    = degRoll;
						
						gps_status_raw_pub_.publish(gps_raw_status);
		                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						//发布IMU
						...
						//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						//发布组合导航数据					
						//gps信号存在  if(status == 42 || status == 52)
						degRoll    = Roll*M_PI/180;              //->X
						degPitch   = Pitch*M_PI/180;             //->y
						degHeading = (360-Heading+90)*M_PI/180;  //->Z
						...
						gps_status.Vy      = Vn;                 
						gps_status.Vz      = Vu;                   
						gps_status.Vv      = VBaseline;
						gps_status.Heading = degHeading;
						gps_status.Pitch   = degPitch;
						gps_status.Roll    = degRoll;

						gps_status_pub_.publish(gps_status);
		                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						//发布IMU
						...
						//角速度  rad/s
						imu_data.angular_velocity.x =  angular_velocity_x*M_PI/180-(check_data.ThetaX);
						imu_data.angular_velocity.y =  angular_velocity_y*M_PI/180-(check_data.ThetaY);
						imu_data.angular_velocity.z =  angular_velocity_z*M_PI/180-(check_data.ThetaZ);

						//线加速度 m/s^2
		                imu_data.linear_acceleration.x =  linear_acceleration_x*gravity-(check_data.linearx);
						imu_data.linear_acceleration.y =  linear_acceleration_y*gravity-(check_data.lineary);
						imu_data.linear_acceleration.z =  linear_acceleration_z*gravity-(check_data.linearz);
		                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
