...
{  
	...
	ros::NodeHandle nh;
	ros::ServiceClient client = nh.serviceClient<crss_vdcar::CanCtrlInfo>("emcb_info");
	crss_vdcar::CanCtrlInfo srv;
	...
	CommOption txOp;      //trans_class
        /* check */
	{
	if (EMCBLibInitialize() == (unsigned int)EMCB_STATUS_NOT_INITIALIZED)
	{
	...
	memset(txData->Data, 0, sizeof(txData->Data));
